@extends('layouts.templates.att.master')
@push('CSS')

@endpush

@section('Content')

<div class="bg-[#fff] p-8 rounded-lg max-w-1000px m-5 shadow-md">
    
    <h2 class="text-2lg font-bold text-center siemreap-regular my-2 pb-3">សាខា កាកបាទក្រហមកម្ពុជា 25 រាជធានី-​ខេត្ត</h2>
    <ul>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កណ្ដាល.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg font-battambang">ខេត្ត កណ្ដាល</span>
            </div>
            <div class="grid grid-rows-2 m-2 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កំពង់ចាម.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កំពង់ចាម</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កំពង់ឆ្នាំង.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កំពងឆ្នាំង</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}" target="_blank">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កំពង់ធំ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កំពង់ធំ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កំពង់ស្ពឺ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កំពង់ស្ពឺ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កំពត.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កំពត</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កែប.png') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កែប</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត កោះកុង.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត កោះកុង</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ក្រចេះ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ក្រចេះ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត តាកែវ.png') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត តាកែវ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ត្បូងឃ្មុំ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ត្បូងឃ្មុំ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត បន្ទាយមានជ័យ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត បន្ទាយមានជ័យ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត បាត់ដំបង.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត បាត់ដំបង</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ប៉ៃលិន.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ប៉ៃលិន</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ពោធិ៍សាត់.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ពោធិ៍សាត់</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ព្រះវិហារ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ព្រះវិហារ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត មណ្ដលគិរី.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត មណ្ឌលគិរី</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត រតនគិរី.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត រតនគិរី</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត សៀមរាប.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត សៀមរាប</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}" target="_blank">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ស្ទឹងត្រែង.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ស្ទឹងត្រែង</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}" target="_blank">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ស្វាយរៀង.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ស្វាយរៀង</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ឧត្តរមានជ័យ.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ឧត្តរមានជ័យ</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      <li class="border-b bg-slate-50 rounded-lg hover:bg-indigo-50 p-2 hover:ring-indigo-200 hover:rounded-lg my-2">
        <a href="{{ route('table_member') }}" target="_blank">
          <div class="flex justify-between items-center">
            <div class="flex items-center">
              <img
              src="{{ asset('images/logo/ខេត្ត ព្រៃវែង.jpg') }}"
              alt="Logo 1"
              class="ml-10 w-16 mr-8 rounded-full object-cover h-16"
              />
              <span class="text-lg siemreap-regular">ខេត្ត ព្រៃវែង</span>
            </div>
            <div class="grid grid-rows-2 m-1 place-items-end content-between gap-8">

              <span class="text-xs siemreap-regular">ស.ម​ <strong>120នាក់</strong></span>
              <span class="text-xs siemreap-regular"> 10 សាខា</span>
              
            </div>
          </div>
        </a>
      </li>
      
      <!-- Add more items as needed -->
    </ul>
  </div>
  @endsection

  @push('JS')

  @endpush